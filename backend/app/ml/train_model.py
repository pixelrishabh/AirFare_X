...
def train_and_save_model():
    ...
    # Directional accuracy test on consecutive test observations
    diff_actual = np.diff(y_test.values[:200])
    diff_pred = np.diff(y_pred[:200])
    dir_acc = float(np.mean(np.sign(diff_actual) == np.sign(diff_pred)) * 100)
    
    metadata = {
        'model_name': 'Airfare Predictor (scikit-learn RandomForest Pipeline)',
        'model_version': 'v1.4-rf-pipeline',
        'algorithm': 'RandomForestRegressor(n_estimators=100, max_depth=15, random_state=42)',
        'preprocessor': 'ColumnTransformer(OneHotEncoder(cat) + StandardScaler(num))',
        ...
        'artifact_file': 'airfare_model.joblib'
    }
    
    # Retrain on full dataset and export
    ...
    print(f'=== TRAINED AND EXPORTED REAL MODEL ===')
    print(f'Artifact: {MODEL_PATH}')
    print(f'Metadata: {METADATA_PATH}')
    print(f'Metrics: MAE={mae:.2f}, RMSE={rmse:.2f}, R2={r2:.4f}, MAPE={mape:.2f}%')
