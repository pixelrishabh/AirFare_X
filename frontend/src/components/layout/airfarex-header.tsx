...
export function AirfarexHeader({ title, showDemoNotice }: AirfarexHeaderProps) {
  const isDemo = import.meta.env.VITE_DEMO_MODE === 'true' || showDemoNotice === true
    ...
    <Header fixed className='border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60'>
      <div className='flex items-center gap-2 mr-auto'>
        ...
        <div className='hidden md:flex items-center gap-1.5 text-xs text-muted-foreground bg-muted/60 px-2.5 py-1 rounded-full border border-border/50'>
          ...
          <span>Pipeline Active ?" Updated Real-Time</span>
        </div>
        {isDemo ? (
          ...
          <Badge variant='outline' className='hidden lg:inline-flex text-[11px] font-medium bg-primary/10 text-primary border-primary/20 gap-1'>
            <IconDatabase className='size-3' />
            AirFareX Research Intelligence
          </Badge>
        )}
      </div>
